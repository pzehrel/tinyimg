...
TinyPNG image compression plugin for Vite.
...
Plugins do not read `.env` files themselves; the build tool (Vite) loads them. It is recommended to use `.env.local` for local secrets.

Supported variable names: any ending with `TINYIMG_KEY`, `TINYIMG_KEYS`, `TINYPNG_KEY`, `TINYPNG_KEYS`. You can also add a prefix, e.g. `VITE_TINYIMG_KEY`.
...
See [`examples/vite`](https://github.com/pzehrel/tinyimg/tree/main/examples/vite) in the repo for a runnable example.
