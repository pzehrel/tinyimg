  ...
  return {
    ...
    async generateBundle(_, bundle) {
        ...
        images.map(([name, asset]) =>
          limit(async () => {
            ...
            if (result.error) {
              ...
              console.error(`${t('status.failed')} ${name.padEnd(40)} ${t('cli.output.failed')} ${errorMsg} (${compressorName})`)
              return
            }
            ...
            this.info(`${t('status.success')} ${name.padEnd(40)} ${origStr}\u2192${compStr}${formatExtras(extras)}`)
          }),
        ),
      )
      ...
      if (images.length > 0) {
        ...
        this.info(summaryParts.join('  '))
      }

      if (convertiblePngs.length > 0) {
        this.warn(t('cli.output.convertiblePngsHint', { count: convertiblePngs.length }))
        this.warn(t('cli.output.convertiblePngsCommand'))
      }

      const projectKeys = listProjectKeys()
      const userKeys = await listUserKeys()
      if (projectKeys.length === 0 && userKeys.length === 0) {
        this.warn(t('cli.output.noKeysHint'))
      }
    },
  }
}
