...
- [ ] **Monorepo 与工程化** (Validated in Phase 1)
  - [x] 使用 pnpm workspace 管理三个包：tinyimg-core、tinyimg-cli、tinyimg-unplugin
  - [x] 统一版本号，发布时同时发布三个包
  - [x] 使用 tsdown 编译 TypeScript，自动生成 .d.ts 类型定义文件
  - [x] 使用 vitest 进行测试
  - [x] 使用 eslint + @antfu/eslint-config（开启 stylistic，禁用 prettier）
  - [x] 最低支持 Node.js 18
  - [x] 使用 GitHub Actions 进行 CI/CD（自动化测试和发布）
...
**Phase 1 Complete** — Monorepo infrastructure established with pnpm workspace, TypeScript configuration, ESLint, Vitest, CI/CD workflows, and Changesets version management. Three packages (tinyimg-core, tinyimg-cli, tinyimg-unplugin) ready for development.

**Next Phase** — Phase 2: 核心 API Key 管理
